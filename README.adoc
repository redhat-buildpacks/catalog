...
[,bash]
----
for entity in pods deployments routes services taskruns pipelineruns applications components snapshotenvironmentbinding.appstudio.redhat.com componentdetectionquery.appstudio.redhat.com; do count=$(kubectl -n $NS get "$entity" -o name | wc -l); echo "$count $entity"; done | sort -n
----
...
- Within the popup window displayed, click on the button `Send PullRequest` and wait a few moment till the PR has ben sent to the Github repository
