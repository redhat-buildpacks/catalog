...
[,bash]
----
for entity in pods deployments routes services taskruns pipelineruns applications components snapshotenvironmentbinding.appstudio.redhat.com componentdetectionquery.appstudio.redhat.com; do count=$( oc -n $NS get "$entity" -o name | wc -l ); echo "$count $entity"; done | sort -n
----
...
steps manually:
- Select for the `quarkus-hello` component `Manage Build pipelines`
- Within the popup windowd displayed, click on the button `Send PullRequest` and wait a few moment till the PR has ben sent to the Github repository
- It is not needed to accept the PR as the project alrady contains a customized tekton folder
